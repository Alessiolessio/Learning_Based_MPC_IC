...
# Output is [x_next, y_next, yaw_sin_next, yaw_cos_next]
TARGET_NAMES = ["x", "y", "yaw_sin", "yaw_cos"]
...
def train_model(model, train_data, val_data, epochs, learning_rate, batch_size, device="cuda",
    ...
    for epoch in range(epochs):
        ...
        comp_sum_abs = torch.zeros(4, device=device)  # [x, y, yaw_sin, yaw_cos]
        n_train = 0
        ...
        model.eval()
        val_loss = 0.0
        comp_sum_abs_v = torch.zeros(4, device=device)  # [x, y, yaw_sin, yaw_cos]
        n_val = 0
