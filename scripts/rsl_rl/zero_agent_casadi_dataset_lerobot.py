...
# >>> LEROBOT (import): biblioteca do formato v3.0
#     Se faltar a lib, instale: `pip install lerobot`
...
def main():
    ...
    # >>> LEROBOT (dt/fps): usa Environment step-size (~0.02 s → 50 Hz)
    DT = 0.02
    FPS = 1.0 / DT
    print(f"[DATASET] Using DT={DT:.5f} s (FPS={FPS:.2f})")
    ...
    # >>> LEROBOT (create dataset): cria o repositório local v3.0
    #     - features descrevem as colunas (shape/dtype) que vamos gravar por frame.
    LOG_DIR_BASE = "/home/nexus/VQ_PMCnmpc/VQ_PMC/logs"
    # inclui milissegundos para evitar colisão se rodar 2x no mesmo segundo
    run_tag = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    DATASET_ROOT = os.path.join(LOG_DIR_BASE, f"terra_sentia_v3_run_{run_tag}")

    # NÃO crie a pasta aqui – deixe o LeRobot criar sozinho
    dataset = LeRobotDataset.create(
        root=DATASET_ROOT,
        repo_id=None,
        fps=FPS,
        features={
            "observation.state": {"shape": [9], "dtype": "float32"},  # [vx,vy,wz,x,y,qw,qx,qy,qz]
            "action":             {"shape": [2], "dtype": "float32"},  # [v,w]
            "timestamp":          {"shape": [1], "dtype": "float32"},  # <= vetor de 1
            "step":               {"shape": [1], "dtype": "int64"},    # <= vetor de 1
        },
    )
    frames_in_episode = 0  # <= contador de frames do episódio corrente
    ...
    # >>> LEROBOT (episódio corrente): contador para depurar (opcional)
    current_episode = 0
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            # >>> LEROBOT (logging por frame)
            #     - timestamp = step_counter * DT (em segundos)
            #     - observation.state com os 10 campos desejados
            #     - action = [v,w] que foi aplicada neste step
            timestamp = np.array([step_counter * DT], dtype=np.float32)  # (1,)
            step_val  = np.array([step_counter], dtype=np.int64)         # (1,)

            if obs is not None:
                try:
                    policy_obs = obs['policy']          # tensor shape (N, 13)
                    ...
                    # Grava um frame por ambiente (N envs)
                    for i in range(num_envs):
                        state_i = torch.stack(
                            [vx[i], vy[i], wz[i], x[i], y[i], qw[i], qx[i], qy[i], qz[i]],
                            dim=0
                        ).detach().cpu().numpy().astype("float32")

                        action_i = torch.stack([act_v[i], act_w[i]], dim=0).detach().cpu().numpy().astype("float32")

                        frame = {
                            "observation.state": state_i,   # (9,) = [vx, vy, wz, x, y, qw, qx, qy, qz]
                            "action": action_i,             # (2,)
                            "timestamp": timestamp,         # (1,)
                            "step": step_val,               # (1,)
                        }
                        dataset.add_frame(frame)  # equivalente a dataset.add_frame(frame)
                        frames_in_episode += 1  # <= incrementa só se add_frame não levantou erro

                    # (restante do histórico para gráficos)
                    euler_obs = current_euler_obs(policy_obs)
                    current_linear_pos = euler_obs[:, [6, 7, 11]]  # (N,3)
                    pos_np = euler_obs[:, [6, 7, 11]].detach().cpu().numpy()   # x,y,theta
                    vel_np = euler_obs[:, [0, 1, 5]].detach().cpu().numpy()    # vx,vy,omega
                    for i in range(num_envs):
                    ...
                    # NÃO engula o erro: mostre qual foi
                    print(f"[DATASET] add_frame falhou: {type(e).__name__}: {e}")
            ...
            if torch.any(done_mask):
                # >>> LEROBOT: fecha episódio antes do reset
                if frames_in_episode > 0:
                    dataset.save_episode()
                    current_episode += 1
                    print(f"[DATASET] Episódio {current_episode} salvo com {frames_in_episode} frames.")
                else:
                    print("[DATASET] Aviso: save_episode() ignorado porque 0 frames foram gravados.")
                ...
                # 3) Reset the env, the PID state, and the step counter for the next episode
                env.reset()
                NMPC.reset()
                step_counter = 0
                frames_in_episode = 0  # <= zera para o próximo episódio

    # >>> LEROBOT: finalize dataset (OBRIGATÓRIO no v3.0)
    if frames_in_episode > 0:
        dataset.save_episode()
        current_episode += 1
        print(f"[DATASET] Episódio final {current_episode} salvo com {frames_in_episode} frames.")

    dataset.finalize()
    print(f"[DATASET] Finalizado em: {DATASET_ROOT}")
